...
sum(amount) as total_amount_core,
sum(amount_usd) as total_amount_usd
FROM validators
WHERE event_name ='delegatedCoin'
group BY 1
ORDER by 1 desc
)
,
...
avg(price) as price
FROM validators
WHERE event_name ='undelegatedCoin'
group BY 1
ORDER by 1 desc
)
SELECT
d.date,
d.number_of_tx + u.number_of_tx as total_daily_tx,
d.total_amount_core - u.total_amount_core as net_core_staked,
d.total_amount_usd - u.total_amount_usd as net_usd_staked,
u.price
FROM delegate_tx d
JOIN undelegated_tx u
on d.date=u.date
ORDER BY 1 DESC
